...
def app_page():
    """
    :return: L'application streamlit sql
    """
    st.markdown(
        """
        <style>
    ...
    if "data" not in os.listdir():
        logging.info("Creating folder data")
        os.mkdir("data")
    ...
    with st.sidebar:
        ...
        # Affichage DataFrame dans sidebar
        st.dataframe(
            df[(df["Theme"] == Themes) & (df["NumeroQuestion"] == numero_questions)][
                ["Tables", "LastReviewed"]
            ]
    ...
    try:
        if user_reponse is not None:
            st.markdown(
                f"## La réponse à la question est {user_reponse.equals(response)}"
            )
            if user_reponse.equals(response):
                st.write("## Bravo!!!")
            else:
    ...
    if st.button("Supprimer mon compte"):
        if st.session_state.email:
            try:
                conn.execute(
                    "DELETE FROM users WHERE email = ?", (st.session_state.email,)
                )
